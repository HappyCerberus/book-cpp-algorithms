...
The \cpp{std::partial_sort_copy} algorithm has the same behaviour as \cpp{std::partial_sort}; however, it does not operate inline. Instead, the algorithm writes the results to a second range.
...
\begin{box-note}
\footnote Example of using \cpp{std::sort} to achieve the same result as in the previous example.
\tcblower
